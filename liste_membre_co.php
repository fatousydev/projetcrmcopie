...
$sql = match ($filter) {
    'active' => "SELECT * FROM membres WHERE statut = 'actif'",
    'inactive' => "SELECT * FROM membres WHERE statut = 'inactif'",
    'both' => "SELECT * FROM membres WHERE statut IN ('actif', 'inactif')",
    default => "SELECT * FROM membres"
};
...
<head>
    ...
    <title>Gestion des Membres - PAMECAS</title>
    <style>
        :root {
...
<nav>
    ...
    <a href="?filter=active" class="btn btn-info">Actifs</a>
    <a href="?filter=inactive" class="btn btn-info">Inactifs</a>
    <a href="?filter=both" class="btn btn-info">Actifs & Inactifs</a>
</div>
    <div class="table-controls">
        ...
        <a href="export_membre.php" class="btn btn-info" onclick="return confirm('Êtes-vous sûr de vouloir exporter les prospects ?')">
...
<div class="table-container">
    <table border="1">
        ...
        <tbody>
                    ...
                    <tr>
                        <?php foreach ($columns as $column): ?>
                            ...
                            <td class="<?= in_array($column, ['commentaires', 'adresse']) ? 'comments-cell' : 'compact-cell' ?> 
                                ...
                                <?php elseif ($column === 'a_beneficie_credit'): ?>
                                    <?= $value ? '✅' : '❌' ?>
                                <?php elseif (in_array($column, ['date_admission', 'created_at', 'updated_at'])): ?>
            ...
            <?php else: ?>
                <tr>
                    <td colspan="<?= count($columns) + 1 ?>" style="text-align: center;">Aucun membre trouvé</td>
                </tr>
            <?php endif; ?>
        </tbody>
    </table>
