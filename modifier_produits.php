...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $plafond_montant = $_POST['montant'];
    $id_produit = $_POST['id_produit'];

    $updateStmt = $pdo->prepare("UPDATE produits SET 
        nom_produit = ?, type_produit = ?, description = ?, conditions = ?, 
        taux_interet = ?, montant = ?, updated_at = NOW() WHERE id = ?");
    
    if ($updateStmt->execute([$nom_produit, $type_produit, $description, $conditions, $taux_interet, $montant, $id_produit])) {
...
<script>
    function openModal(id, nom, type, description, conditions, taux, plafond) {
        ...
        document.getElementById('montant').value = plafond;
    }
